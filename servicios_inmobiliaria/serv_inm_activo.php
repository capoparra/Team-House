...
// Consulta para obtener todos los servicios inmobiliarios 
$query = "SELECT * FROM tbl_serv_inm";
...
if (isset($_GET['update'])) {
    ...
    if ($update_type === 'success') {
        $mensaje = 'El servicios inmobiliaria ha sido actualizado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'error') {
        $mensaje = 'Hubo un error al actualizar del servicio inmobiliaria .';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_success') {
        $mensaje = 'El servicio inmobiliaria  ha sido inactivado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_error') {
        $mensaje = 'Hubo un error al inactivar el servicio inmobiliaria.';
        $modal_visible = true;
    } elseif ($update_type === 'reactivate_success') {
        $mensaje = 'El servicios inmobiliaria  ha sido Reactivado exitosamente.';
        $modal_visible = true;
    }
}
...
<body>
    ...
    <nav class="navbar bg-white mb-5" style="--bs-bg-opacity: 0.2; backdrop-filter:blur(1rem); 
        ...
        <div class="container-fluid">
            <form class="d-flex" role="search" method="GET" action="serv_inm_activo.php">
                <input type="search" name="buscar" class="form-control me-2"
                    placeholder="Buscar por inmueble, nombre o contrato" style="background-color: #E4C1FF; width: 20rem">
                <button type="submit" name="btn" class="btn btn-outline-light"
                    style="border-radius: 2rem">Buscar</button>
                <a href="serv_inm_activo.php" class="text-light ms-2 my-auto" style="font-size: 0.85rem;">Limpiar
                    Vista</a>
            </form>
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a class="btn fw-bold my-2 me-5 btn-outline-transparent text-dark" href="registro_serv_inm.php"
                    role="button" style="background: linear-gradient(-90deg, #C170FF, #E6C5FF); border-radius: 2rem">
                    <i class="bi bi-plus-circle me-2"></i> CREAR SERVICIO inmobiliaria
            </div>
        </div>
    </nav>
    <nav class="navbar bg-transparent navbar-expand-lg mt-5 mb-3 ms-5 fixed">
        <div class="container-fluid">
            <h5 class="text-light"><i class="bi bi-list-check text-light" style="padding: 1rem"></i> Lista de
                servicios inmuebles activos</h5>
        </div>
    </nav>
    ...
    <div class="container">
        <div class="row justify-content-center">
            ...
            if (isset($_GET['btn'])) {
                ...
                $stmt = $conn->prepare(
                    "SELECT * FROM tbl_serv_inm
                    WHERE (inmub_codigo LIKE ? OR nombre_cliente LIKE ? OR id_cliente LIKE ?) 
                    AND estado_serv = 1
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $estado = ($row['estado_serv_inm'] == 1) ? 'Activo' : 'Inactivo';
                        echo '
                        <div class="col-md-4">    
                            <div class="card mt-3 mb-2 mx-auto bg-dark" style="width: 23rem; height: 18rem; padding: 10px;  --bs-bg-opacity: 0.7; ">
                                ...
                                <p class="card-text text-light"> 
                                    ...
                                    <strong>Identificación:</strong> &nbsp ' . htmlspecialchars($row['num_contrato'] . ' ' . $row['id_cliente']) . '
                                </p>
                                <p class="card-text text-light">
                                        <i class="bi bi-phone-vibrate me-1"></i>
                                        <strong>Teléfono:</strong> &nbsp ' . htmlspecialchars($row['nombre_cliente']) . '
                                </p>
                                ...
                                <div class="btn-group dropend" style="position: relative; z-index: 1050;">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-dark" style="position: absolute" aria-labelledby="dropdownMenuButton">
                                        ...
                                        <li><a class="dropdown-item" href="actualizar_serv_inm.php?id=' . $row['nombre_cliente'] . '">Editar Información</a></li>
                                        <li>
                                            <!-- Formulario oculto para inactivar -->
                                            <form action="inactivar_arr.php" method="POST" style="display:inline;">
                                                <input type="hidden" name="id" value="' . htmlspecialchars($row['nombre_cliente']) . '">
