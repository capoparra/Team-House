...
// Consulta para obtener todos los servicios publicos 
$query = "SELECT * FROM tbl_serv_publicos";
...
if (isset($_GET['update'])) {
    ...
    if ($update_type === 'success') {
        $mensaje = 'El servicios publico ha sido actualizado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'error') {
        $mensaje = 'Hubo un error al actualizar del servicio publico .';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_success') {
        $mensaje = 'El servicio publico  ha sido inactivado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_error') {
        $mensaje = 'Hubo un error al inactivar el servicio publico.';
        $modal_visible = true;
    } elseif ($update_type === 'reactivate_success') {
        $mensaje = 'El servicios publico  ha sido Reactivado exitosamente.';
        $modal_visible = true;
    }
}
    ...
    echo '<div class="container">
            ...
            box-shadow: 1.3rem 1.3rem 1.3rem rgba(0,0,0,0.5);">
                        ...
                        <div class="modal-footer">
                            <a href="serv_publicos_activo.php">
...
<body>
    ...
    <nav class="navbar bg-white mb-5" style="--bs-bg-opacity: 0.2; backdrop-filter:blur(1rem); 
        ...
        <div class="container-fluid">
            <form class="d-flex" role="search" method="GET" action="serv_publicos_activo.php">
                ...
                <a href="serv_publicos_activo.php" class="text-light ms-2 my-auto" style="font-size: 0.85rem;">Limpiar
                    Vista</a>
            </form>
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a class="btn fw-bold my-2 me-5 btn-outline-transparent text-dark" href="registro_serv_publicos.php"
                    role="button" style="background: linear-gradient(-90deg, #C170FF, #E6C5FF); border-radius: 2rem">
                    <i class="bi bi-plus-circle me-2"></i> CREAR SERVICIO PUBLICO   
                </a>
            </div>
        </div>
    </nav>
            ...
            if (isset($_GET['btn'])) {
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $estado = ($row['estado_serv_publicos'] == 1) ? 'Activo' : 'Inactivo';
                        echo '
                        <div class="col-md-4">    
                            <div class="card mt-3 mb-2 mx-auto bg-dark" style="width: 23rem; height: 18rem; padding: 10px;  --bs-bg-opacity: 0.7; ">
                                ...
                                <div class="btn-group dropend" style="position: relative; z-index: 1050;">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-dark" style="position: absolute" aria-labelledby="dropdownMenuButton">
                                        ...
                                        <li><a class="dropdown-item" href="actualizar_serv_publicos.php?id=' . $row['num_identificacion'] . '">Editar Información</a></li>
                                        <li>
