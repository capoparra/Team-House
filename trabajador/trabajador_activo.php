...
$query = "SELECT * FROM tbl_trabajador";
...
if (isset($_GET['update'])) {
    ...
    if ($update_type === 'success') {
        $mensaje = 'El trabajador ha sido actualizado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'error') {
        $mensaje = 'Hubo un error al actualizar el trabajador.';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_success') {
        $mensaje = 'El trabajador ha sido inactivado exitosamente.';
        $modal_visible = true;
    } elseif ($update_type === 'inactivate_error') {
        $mensaje = 'Hubo un error al inactivar el trabajador.';
        $modal_visible = true;
    } elseif ($update_type === 'reactivate_success') {
        $mensaje = 'El trabajador ha sido Reactivado exitosamente.';
        $modal_visible = true;
    }
}
    ...
    echo '<div class="container">
            ...
            box-shadow: 1.3rem 1.3rem 1.3rem rgba(0,0,0,0.5);">
                        ...
                        <div class="modal-footer">
                            <a href="arrendatario_activo.php">
...
<body>
    ...
    <nav class="navbar bg-white mb-5" style="--bs-bg-opacity: 0.2; backdrop-filter:blur(1rem); 
        ...
        <div class="container-fluid">
            <form class="d-flex" role="search" method="GET" action="trabajador_activo.php">
                ...
                <a href="trabajador_activo.php" class="text-light ms-2 my-auto" style="font-size: 0.85rem;">Limpiar
                    Vista</a>
            </form>
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a class="btn fw-bold my-2 me-5 btn-outline-transparent text-dark" href="form_trabajador.php"
                    role="button" style="background: linear-gradient(-90deg, #C170FF, #E6C5FF); border-radius: 2rem">
                    <i class="bi bi-plus-circle me-2"></i> CREAR TRABAJADOR
                </a>
            </div>
        </div>
    </nav>
    ...
    <div class="container">
        <div class="row justify-content-center">
            ...
            if (isset($_GET['btn'])) {
                $buscar = '%' . $_GET['buscar'] . '%';
                // Preparar la consulta SQL para buscar trabajador activos
                $stmt = $conn->prepare(
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $estado = ($row['estado_tra'] == 1) ? 'Activo' : 'Inactivo';
                        echo '
                        <div class="col-md-4">    
                            <div class="card mt-3 mb-2 mx-auto bg-dark" style="width: 23rem; height: 18rem; padding: 10px;  --bs-bg-opacity: 0.7; ">
                                ...
                                <div class="btn-group dropend" style="position: relative; z-index: 1050;">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-dark" style="position: absolute" aria-labelledby="dropdownMenuButton">
                                        ...
                                        <li><a class="dropdown-item" href="actualizar_trabajador.php?id=' . $row['num_identificacion'] . '">Editar Información</a></li>
                                        <li>
